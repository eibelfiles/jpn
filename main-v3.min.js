...
function $$(e){return document.getElementById(e)}function $(e,t){return(t||document).querySelector(e)}function toSeconds(e){var t=e.split(":");return 60*+t[0]*60+60*+t[1]}function getTrackPosition(e,t){return(toSeconds(e)-toSeconds(t))/16+48}function setAccentColor(e,t){let n=new ColorThief;t.complete?e.setAttribute("style",`--accent: rgb(${n.getColor(t)})`):t.addEventListener("load",(function(){e.setAttribute("style",`--accent: rgb(${n.getColor(t)})`)}))}function setScrollText(){document.querySelectorAll(".player-meta").forEach((e=>{var t=$(".song-title",e),n=t.offsetWidth,a=e.offsetWidth;e.setAttribute("style","--title-width:"+a+"px"),n>a?t.classList.add("song-very-long"):t.classList.remove("song-very-long")}))}const DATE_UNITS={day:86400,hour:3600,minute:60,second:1},getSecondsDiff=e=>(Date.now()-e)/1e3,getUnitAndValueDate=e=>{for(const[t,n]of Object.entries(DATE_UNITS))if(e>=n||"second"===t){return{value:-1*Math.floor(e/n),unit:t}}},getTimeAgo=e=>{const t=new Intl.RelativeTimeFormat,n=getSecondsDiff(e),{value:a,unit:s}=getUnitAndValueDate(n);return t.format(a,s)};function createUpcoming(e){var t="";for(const n of e)t+=`\n      <div class="upcoming bg-dark shadow-xl" data-start="${n.start}" data-end="${n.end}">\n        <img class="upcoming-image fit-image" src="${n.poster}" alt="${n.title}">\n        <div class="upcoming-data fs-7 relative py-.75 px-1.25">\n          <div class="upcoming-title fw-500 color-title nowrap">${n.title}</div>\n          <div class="upcoming-time">${n.start} - ${n.end}</div>\n        </div>\n      </div>\n    `;return t}const upcoming=$$("renderUpcoming");function createHistory(e,t){let n="";for(const a of e.slice(0,t)){let e=a.played_at,t=new Date(1e3*e);n+=`\n      <div class="history-item relative">\n        <img oncontextmenu="return false;" class="events-none history-image bg-dark flex-none rounded-.5 shadow-m mr-1" src="${a.song.art}" alt="${a.song.text}" width="120" height="120">\n        <div class="history-meta flex-1">\n          <h3 class="fs-6 fw-500 color-title">${a.song.title}</h3>\n          <div class="fs-7 opacity-.75">${a.song.artist}</div>\n          <div class="fs-7 opacity-.75">${getTimeAgo(t)}</div>\n        </div>\n      </div>\n    `}return n}function setPlayerMeta(e,t){let n=$(".song-cover",e),a=$(".song-title",e),s=$(".song-artist",e),o=$(".song-album",e);n&&(n.src=t.art),a&&(a.innerText=t.title),s&&(s.innerText=t.artist),o&&(o.innerText=t.album)}upcoming&&(upcoming.innerHTML=createUpcoming(currentSchedule));const player=$(".player"),audioPlayer=$(".player-audio");if(audioPlayer&&audioPlayer.dataset.src){const e=new Audio(audioPlayer.dataset.src),t=$(".player-volume",audioPlayer);t.addEventListener("change",(function(t){e.volume=t.currentTarget.value/100,localStorage.setItem("player_vol",e.volume)}),!1);var getVolume=localStorage.getItem("player_vol");getVolume&&(e.volume=getVolume,t.value=100*getVolume);var playBtn=$(".player-toggle",audioPlayer),playIcon=$(".i-play",audioPlayer),pauseIcon=$(".i-pause",audioPlayer);function setPlayStatus(){e.load(),player.classList.toggle("is-playing"),e.play(),playBtn.innerHTML='<svg class="i i-pause" viewBox="0 0 24 24"><path d="M5 4h4v16H5Zm10 0h4v16h-4Z"></path></svg>'}function setPauseStatus(){player.classList.toggle("is-playing"),e.pause(),playBtn.innerHTML='<svg class="i i-play" viewBox="0 0 24 24"><path d="m7 3 14 9-14 9z"></path></svg>'}"mediaSession"in navigator&&(navigator.mediaSession.setActionHandler("play",(function(){setPlayStatus()})),navigator.mediaSession.setActionHandler("pause",(function(){setPauseStatus()}))),playBtn.addEventListener("click",(()=>{e.paused?setPlayStatus():setPauseStatus()}),!1)}const japanisu="https://stream.radioani.me/api/nowplaying_static/japanisu.json";function playerInit(){fetch(japanisu).then((e=>e.json())).then((e=>{let t=e.song_history,n=$$("playerHistory"),a=$(".song-now",player),s=$(".song-next",player),o=e.now_playing.remaining,i=$(".player-poster");i&&i.src&&(i.crossOrigin="Anonymous",i.src="https://images.weserv.nl/?url="+encodeURIComponent(e.now_playing.song.art),setAccentColor(document.body,i)),a&&setPlayerMeta(a,e.now_playing.song),s&&setPlayerMeta(s,e.playing_next.song),n&&(n.innerHTML=createHistory(t,n.dataset.results||5)),"mediaSession"in navigator&&(navigator.mediaSession.metadata=new MediaMetadata({title:e.now_playing.song.title,artist:e.now_playing.song.artist,album:e.now_playing.song.album,artwork:[{src:e.now_playing.song.art,sizes:"96x96",type:"image/png"},{src:e.now_playing.song.art,sizes:"128x128",type:"image/png"},{src:e.now_playing.song.art,sizes:"192x192",type:"image/png"},{src:e.now_playing.song.art,sizes:"256x256",type:"image/png"}]})),setScrollText(),setTimeout(playerInit,1e3*o)})).catch((e=>console.log(e)))}playerInit();const listeners=$$("listeners");function getListeners(){fetch(japanisu).then((e=>e.json())).then((e=>{let t=e.listeners.current;listeners.innerText=t,setTimeout(getListeners,1e3*(listeners.dataset.update||5))})).catch((e=>console.log(e)))}listeners&&getListeners();const program=$$("program");function setProgramStatus(){let e=new Date,t=e.toLocaleTimeString([],{timeStyle:"short",timeZone:"America/Hermosillo"});console.log(t);let n=e.toLocaleString("es",{weekday:"long"}),a=program.querySelectorAll(".program-item"),s=toSeconds(t);a.forEach((e=>{if(e.dataset.week==n){e.querySelectorAll(".upcoming").forEach((e=>{let t=toSeconds(e.dataset.start),n=toSeconds(e.dataset.end);t<=s&&n>s?e.classList.add("is-online"):e.classList.remove("is-online")}))}}))}program&&(setProgramStatus(),setInterval(setProgramStatus,1e4));const programming=$$("renderUpcoming");function setTimePosition(){let e=(new Date).toLocaleTimeString([],{timeStyle:"short",timeZone:"America/Hermosillo"});console.log(e),programming.querySelectorAll(".upcoming").forEach((t=>{let n=getTrackPosition(t.dataset.start,e);t.setAttribute("style",`transform: translateX(${n}px)`),t.classList.add("is-loaded")}))}programming&&(setTimePosition(),setInterval(setTimePosition,6e4));const stickyHeader=document.querySelector(".header-player");function sticky(){if(stickyHeader){var e=stickyHeader.offsetHeight;window.addEventListener("scroll",(function(){window.scrollY>stickyHeader.offsetTop+e?(stickyHeader.nextElementSibling.setAttribute("style","margin-top:"+e+"px"),stickyHeader.classList.add("header-sticky","fixed","z-100","t-0","x-0")):(stickyHeader.classList.remove("header-sticky","fixed","z-100","t-0","x-0"),stickyHeader.nextElementSibling.removeAttribute("style"))}))}}window.innerWidth>760&&sticky();