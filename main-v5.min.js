...
function $$(e){return document.getElementById(e)}function $(e,t){return(t||document).querySelector(e)}function toSeconds(e){var t=e.split(":");return 60*+t[0]*60+60*+t[1]}function setAccentColor(e,t){let n=new ColorThief;t.complete?e.setAttribute("style",`--accent: rgb(${n.getColor(t)})`):t.addEventListener("load",(function(){e.setAttribute("style",`--accent: rgb(${n.getColor(t)})`)}))}function setScrollText(){document.querySelectorAll(".player-meta").forEach((e=>{var t=$(".song-title",e),n=t.offsetWidth,s=e.offsetWidth;e.setAttribute("style","--title-width:"+s+"px"),n>s?t.classList.add("song-very-long"):t.classList.remove("song-very-long")}))}const DATE_UNITS={day:86400,hour:3600,minute:60,second:1},getSecondsDiff=e=>(Date.now()-e)/1e3,getUnitAndValueDate=e=>{for(const[t,n]of Object.entries(DATE_UNITS))if(e>=n||"second"===t){return{value:-1*Math.floor(e/n),unit:t}}},getTimeAgo=e=>{const t=new Intl.RelativeTimeFormat,n=getSecondsDiff(e),{value:s,unit:a}=getUnitAndValueDate(n);return t.format(s,a)};function getNextDay(e){var t="";if("lunes"==e)t="martes";else if("martes"==e)t="miércoles";else if("miércoles"==e)t="jueves";else if("jueves"==e)t="viernes";else if("viernes"==e)t="sábado";else if("sábado"==e)t="domingo";else if("domingo"==e)t="lunes";return t}function getTimeZone(e="America/Hermosillo"){let t=new Date,n=t.toLocaleTimeString([],{timeStyle:"short",timeZone:e}),s=t.toLocaleString("es",{weekday:"long",timeZone:e});return{date:t,time:n,week:s}}function createHistory(e,t){let n="";for(const s of e.slice(0,t)){let e=s.played_at,t=new Date(1e3*e);n+=`\n      <div class="history-item relative">\n        <img oncontextmenu="return false;" class="events-none history-image bg-dark flex-none rounded-.5 shadow-m mr-1" src="${s.song.art}" alt="${s.song.text}" width="120" height="120">\n        <div class="history-meta flex-1">\n          <h3 class="fs-6 fw-500 color-title">${s.song.title}</h3>\n          <div class="fs-7 opacity-.75">${s.song.artist}</div>\n          <div class="fs-7 opacity-.75">${getTimeAgo(t)}</div>\n        </div>\n      </div>\n    `}return n}function setPlayerMeta(e,t){let n=$(".song-cover",e),s=$(".song-title",e),a=$(".song-artist",e),i=$(".song-album",e);n&&(n.src=t.art),s&&(s.innerText=t.title),a&&(a.innerText=t.artist),i&&(i.innerText=t.album)}const player=$(".player"),audioPlayer=$(".player-audio");if(audioPlayer&&audioPlayer.dataset.src){const e=new Audio(audioPlayer.dataset.src),t=$(".player-volume",audioPlayer);t.addEventListener("change",(function(t){e.volume=t.currentTarget.value/100,localStorage.setItem("player_vol",e.volume)}),!1);var getVolume=localStorage.getItem("player_vol");getVolume&&(e.volume=getVolume,t.value=100*getVolume);var playBtn=$(".player-toggle",audioPlayer),playIcon=$(".i-play",audioPlayer),pauseIcon=$(".i-pause",audioPlayer);function setPlayStatus(){e.load(),player.classList.toggle("is-playing"),e.play(),playBtn.innerHTML='<svg class="i i-pause" viewBox="0 0 24 24"><path d="M5 4h4v16H5Zm10 0h4v16h-4Z"></path></svg>'}function setPauseStatus(){player.classList.toggle("is-playing"),e.pause(),playBtn.innerHTML='<svg class="i i-play" viewBox="0 0 24 24"><path d="m7 3 14 9-14 9z"></path></svg>'}"mediaSession"in navigator&&(navigator.mediaSession.setActionHandler("play",(function(){setPlayStatus()})),navigator.mediaSession.setActionHandler("pause",(function(){setPauseStatus()}))),playBtn.addEventListener("click",(()=>{e.paused?setPlayStatus():setPauseStatus()}),!1)}const japanisu="https://stream.radioani.me/api/nowplaying_static/japanisu.json",historyPlayer=$$("playerHistory");function playerInit(){fetch(japanisu).then((e=>e.json())).then((e=>{let t=e.song_history,n=$(".song-now",player),s=$(".song-next",player),a=e.now_playing.remaining,i=$(".player-poster");i&&i.src&&(i.crossOrigin="Anonymous",i.src="https://images.weserv.nl/?url="+encodeURIComponent(e.now_playing.song.art),setAccentColor(document.body,i)),n&&setPlayerMeta(n,e.now_playing.song),s&&setPlayerMeta(s,e.playing_next.song);var o=recentlyBtn.classList.contains("is-active")?15:4;historyPlayer&&(recentlyBtn.onclick=()=>{recentlyBtn.classList.contains("is-active")?(recentlyBtn.classList.remove("is-active"),recentlyBtn.innerText="Ver más",historyPlayer.innerHTML=createHistory(t,4)):(recentlyBtn.classList.add("is-active"),recentlyBtn.innerText="Ver menos",historyPlayer.innerHTML=createHistory(t,15))},historyPlayer.innerHTML=createHistory(t,o)),"mediaSession"in navigator&&(navigator.mediaSession.metadata=new MediaMetadata({title:e.now_playing.song.title,artist:e.now_playing.song.artist,album:e.now_playing.song.album,artwork:[{src:e.now_playing.song.art,sizes:"96x96",type:"image/png"},{src:e.now_playing.song.art,sizes:"128x128",type:"image/png"},{src:e.now_playing.song.art,sizes:"192x192",type:"image/png"},{src:e.now_playing.song.art,sizes:"256x256",type:"image/png"}]})),setScrollText(),setTimeout(playerInit,1e3*a)})).catch((e=>console.log(e)))}playerInit();const listeners=$$("listeners");function getListeners(){fetch(japanisu).then((e=>e.json())).then((e=>{let t=e.listeners.current;listeners.innerText=t,setTimeout(getListeners,1e3*(listeners.dataset.update||5))})).catch((e=>console.log(e)))}function upcomingItems(e){return`\n    <div class="upcoming bg-dark shadow-xl" data-start="${e.start}" data-end="${e.end}" data-weekday="${e.day}" >\n      <img class="upcoming-image fit-image" src="${e.poster}" alt="${e.title}">\n      <div class="upcoming-data fs-7 relative py-.75 px-1.25">\n        <div class="upcoming-title fw-500 color-title nowrap">${e.title}</div>\n        <div class="upcoming-snippet fw-500 my-1">${e.snippet}</div>\n        <div class="upcoming-time">${e.start} - ${e.end}</div>\n      </div>\n    </div>\n  `}function createUpcoming(e){let t=getTimeZone();var n="";let s=getNextDay(t.week);return e.forEach((e=>{e.day!=t.week&&e.day!=s||(n+=upcomingItems(e))})),n}function createWeekely(e,t){var n="";return e.forEach((e=>{e.day==t&&(n+=upcomingItems(e))})),n}listeners&&getListeners();const program=$$("program"),programBtn=$$("showWeeklySchedule"),recentlyBtn=$$("showRecentlyPlayed");function setProgramStatus(){let e=getTimeZone(),t=program.querySelectorAll(".program-program"),n=toSeconds(e.time);t.forEach((t=>{if(t.dataset.week==e.week){t.querySelectorAll(".upcoming").forEach((e=>{let t=toSeconds(e.dataset.start),s=toSeconds(e.dataset.end);t<=n&&s>n?e.classList.add("is-online"):e.classList.remove("is-online")}))}}))}function createItemsProgram(){program.classList.add("program","grid","l:cols-7","cols-1","is-active"),program.querySelectorAll(".program-program").forEach((e=>{let t=e.dataset.week;e.innerHTML=createWeekely(currentSchedule,t)})),program.classList.remove("none")}function removeItemsProgram(){program.classList.add("none"),program.classList.remove("program","grid","l:cols-7","cols-1","is-active"),program.querySelectorAll(".program-program").forEach((e=>{e.innerHTML=""}))}program&&programBtn.addEventListener("click",(()=>{programBtn.classList.contains("is-active")?(programBtn.classList.remove("is-active"),removeItemsProgram(),clearInterval(setProgramStatus)):(programBtn.classList.add("is-active"),createItemsProgram(),setProgramStatus(),setInterval(setProgramStatus,1e4))}),!1);const programming=$$("renderUpcoming");function setTimePosition(){let e=getTimeZone();programming.querySelectorAll(".upcoming").forEach((t=>{let n=(toSeconds(t.dataset.start)-toSeconds(e.time))/16+48,s=(toSeconds(t.dataset.start)-(toSeconds(e.time)-86400))/16+48;t.setAttribute("style",`transform: translateX(${t.dataset.weekday==e.week?n:s}px)`),t.classList.add("is-loaded")}))}function setUpcomingItems(){let e=getTimeZone(),t=toSeconds("24:00")-toSeconds(e.time);console.log("se ejecuta de nuevo en:"+t),programming&&(programming.innerHTML=createUpcoming(currentSchedule)),setTimePosition(),setTimeout(setUpcomingItems,1e3*(0===t?10:t))}programming&&(setTimePosition(),setInterval(setTimePosition,2e4)),setUpcomingItems();const stickyHeader=document.querySelector(".header-player");function sticky(){if(stickyHeader){var e=stickyHeader.offsetHeight;window.addEventListener("scroll",(function(){window.scrollY>stickyHeader.offsetTop+e?(stickyHeader.nextElementSibling.setAttribute("style","margin-top:"+e+"px"),stickyHeader.classList.add("header-sticky","fixed","z-100","t-0","x-0")):(stickyHeader.classList.remove("header-sticky","fixed","z-100","t-0","x-0"),stickyHeader.nextElementSibling.removeAttribute("style"))}))}}window.innerWidth>760&&sticky();